...
Bul "Importar marcos do TAP: na secao Marcos de cada linha, o botao `Importar marcos do TAP` (visivel com um Projeto vinculado selecionado) traz os marcos do cronograma de entregas macro do TAP daquele projeto - o nome do marco vira o Nome do marco aqui, o Termino previsto vira a Data, e o Status comeca sempre como `A iniciar` (o TAP nao registra se um marco ja foi concluido). Marcos com o mesmo nome que ja estiverem na lista nao sao duplicados - o botao pode ser clicado varias vezes com seguranca, assim como o `Importar do TAP` dos Indicadores (secao 4.2)."
...
Nota "Como o selo de saude e calculado: compara tres sinais - a situacao do projeto esta `Atrasado` no Relatorio de Situacao; existe pelo menos um marco ativo (nao Concluido/Cancelado) cuja data-alvo ja passou (mesma definicao usada no indicador `Marcos no prazo` do Relatorio de Resultados, secao 6.7 - o que conta e a data-alvo, nao o status manual do marco); e o custo realizado ultrapassa o estimado no TAP. Dois ou mais sinais ruins = Critico; um sinal = Atencao; nenhum = Normal. Projetos Concluido/Cancelado mostram `Encerrado`, e projetos sem nenhuma linha no Relatorio de Situacao mostram `Sem dados` - nunca inventa um status a partir de dado ausente. A mesma coluna `Saude` aparece na tabela `Visao atual por projeto` do Painel Executivo (secao 6.4), com o mesmo calculo."
...
Bul "Marcos no prazo: % dos marcos de projetos ainda ativos no Relatorio de Situacao (secao 4.1) que NAO estao atrasados (data-alvo ainda nao passou, ou o marco ja esta Concluido/Cancelado)."
...
Nota "Marcos no prazo nao e o mesmo que `concluido no prazo`: o sistema guarda a data-alvo do marco, nao uma data separada de quando ele foi de fato concluido - por isso o indicador mede ausencia de atraso atual, o dado mais honesto disponivel hoje. Da mesma forma, Projetos dentro do orcamento e Entraves resolvidos aparecem como travessao (`-`) quando nao ha dado suficiente para calcular, em vez de mostrar um numero sem base."
