...
Bul "Saude do projeto: selo geral (Normal/Atencao/Critico/Encerrado/Sem dados), % de marcos concluidos, atraso do marco mais atrasado (em dias) e % do orcamento realizado frente ao estimado no TAP - ver detalhe do calculo logo abaixo."
...
Nota "Como o selo de saude e calculado: compara tres sinais - a situacao do projeto esta `Atrasado` no Relatorio de Situacao; existe pelo menos um marco do cronograma do Planejamento, sem Conclusao real preenchida, cuja data-alvo ja passou (mesma definicao usada no indicador `Marcos no prazo` do Relatorio de Resultados, secao 6.7 - o que conta e a data-alvo e a conclusao real, nao o status manual do marco); e o custo realizado ultrapassa o estimado no TAP. Dois ou mais sinais ruins = Critico; um sinal = Atencao; nenhum = Normal. Projetos Concluido/Cancelado mostram `Encerrado`, e projetos sem nenhuma linha no Relatorio de Situacao mostram `Sem dados` - nunca inventa um status a partir de dado ausente. A mesma coluna `Saude` aparece na tabela `Visao atual por projeto` do Painel Executivo (secao 6.4), com o mesmo calculo."
