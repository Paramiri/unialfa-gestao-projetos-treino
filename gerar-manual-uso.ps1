...
$r3 = @(
  ...
  @("Cronograma de entregas macro (tabela)","Nao","Marco, responsavel, inicio e termino previstos, conclusao real, custo - e o cronograma oficial do projeto (Passo 4 e secao 4.1 mostram este mesmo cronograma, so leitura)"),
...
P "Cronograma de entregas macro (secao 10 do formulario): e o unico cronograma do projeto - o Planejamento (Passo 4) e o Relatorio de Situacao (secao 4.1) mostram estes mesmos marcos, sempre ao vivo e so leitura, sem manter copia propria. Alem de Inicio e Termino previstos, cada marco tem um campo `Conclusao real`, preenchido quando o marco de fato termina - e o que permite ao sistema distinguir um marco `atrasado agora` de um marco que `terminou atrasado`: um marco com Conclusao real preenchida nunca conta como atrasado no calculo de Saude do projeto (secao 6.4.1) nem no indicador Marcos no prazo (secao 6.7), mesmo que o Termino previsto ja tenha passado."
P "Dependencia entre marcos: cada marco tambem tem um campo opcional `Depende de`, onde se digita o nome de outro marco do mesmo cronograma. Se o marco indicado ainda nao tiver Conclusao real e o Termino previsto ja tiver passado, o marco dependente ganha um aviso `bloqueado` - visivel no proprio TAP (edicao, detalhes e impressao/exportacao), no espelho do Planejamento e nos Marcos do Relatorio de Situacao. E uma verificacao simples, de um nivel so e por comparacao de nome - nao calcula um caminho critico completo nem cadeias de varios marcos."
...
$r4 = @(
  ...
  @("3. Cronograma","So leitura: mostra o cronograma de entregas macro ja registrado no TAP do mesmo projeto (marco, responsavel, inicio, termino, conclusao real, custo) - para alterar um marco, edite o TAP"),
...
Nota "Projeto vinculado (Gate 1) - opcional: cada linha de projeto pode, opcionalmente, ser associada a um Projeto ja Aprovado no Gate 1 (o mesmo cadastro compartilhado usado pelo Canvas, TAP, Planejamento, EAP, SMP, TEP e RLA - secao 2.5). Ao selecionar um projeto na lista, os campos Nome do projeto, Responsavel (a partir do gestor do projeto) e Inicio previsto/Termino previsto (a partir da menor e maior data do cronograma de entregas macro do TAP vinculado, se houver) sao preenchidos automaticamente - sempre so nos campos ainda vazios, sem sobrescrever o que ja foi digitado. Tambem aparece o botao `Ver historico`, que mostra as alteracoes desse projeto vindas de qualquer outro formulario vinculado a ele. Deixar em `Nenhum` mantem o comportamento anterior - o projeto do Relatorio de Situacao continua sendo texto livre, sem vinculo. Se o TAP nao tiver nenhuma data de cronograma preenchida, Inicio previsto e Termino previsto ficam em branco para preenchimento manual - o sistema nao tenta adivinhar a partir de outro documento."
...
Bul "Marcos vem do TAP: a secao Marcos de cada linha de projeto mostra, automaticamente (sem botao de importar), os marcos do cronograma de entregas macro do TAP daquele projeto - nome, Inicio/Termino previsto e Conclusao real vem sempre ao vivo do TAP (para mudar um marco, edite o TAP). O unico campo proprio aqui e o Status de cada marco (A iniciar, No prazo, Atrasado, Paralisado, Concluido ou Cancelado), que continua sendo ajustado manualmente."
...
Bul "Cronograma e custos planejados (TAP): o cronograma de entregas macro e o custo total estimado do TAP desse projeto, para comparar com os custos realizados do Relatorio de Situacao."
...
Img "57_ficha_projeto.png" "Ficha do Projeto: dados do cadastro, saude do projeto, situacao atual, progresso do ciclo de vida e cronograma do TAP." 5.8
Nota "Como o selo de saude e calculado: compara tres sinais - a situacao do projeto esta `Atrasado` no Relatorio de Situacao; existe pelo menos um marco do cronograma do TAP, sem Conclusao real preenchida, cuja data-alvo ja passou (mesma definicao usada no indicador `Marcos no prazo` do Relatorio de Resultados, secao 6.7 - o que conta e a data-alvo e a conclusao real, nao o status manual do marco); e o custo realizado ultrapassa o estimado no TAP. Dois ou mais sinais ruins = Critico; um sinal = Atencao; nenhum = Normal. Projetos Concluido/Cancelado mostram `Encerrado`, e projetos sem nenhuma linha no Relatorio de Situacao mostram `Sem dados` - nunca inventa um status a partir de dado ausente. A mesma coluna `Saude` aparece na tabela `Visao atual por projeto` do Painel Executivo (secao 6.4), com o mesmo calculo."
...
Bul "Marcos no prazo: % dos marcos de projetos ainda ativos no Relatorio de Situacao (secao 4.1) que NAO estao atrasados - a data-alvo (do cronograma do TAP) ainda nao passou, o marco ja tem Conclusao real preenchida no TAP, ou o marco esta marcado como Concluido/Cancelado."
