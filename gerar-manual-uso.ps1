...
P "Responsavel (conta): alem do campo de texto livre Responsavel, cada marco tem um segundo campo opcional, `Responsavel (conta)` - um seletor com as contas que fazem parte da equipe do projeto selecionado (secao 2.5). Diferente do texto livre, que e so uma anotacao, esse campo vincula o marco a uma conta de verdade: e a partir dele (mais o papel Gerente de Projetos da equipe do projeto) que o sistema sabe para quem mandar o aviso automatico de prazo (secao 6.4.3) e para popular o Painel de Prazos (secao 6.4.2). Preencher esse campo e opcional - um marco sem `Responsavel (conta)` continua funcionando normalmente, so nao gera aviso individual."
...
P "Quem recebe cada aviso: a conta vinculada no novo campo `Responsavel (conta)` do cronograma (Passo 4, aba 3), quando preenchido, mais toda conta com papel Gerente de Projetos que estiver na equipe daquele projeto (secao 2.5) - nao o campo de texto livre `Gerente` do cadastro do projeto. Se nenhum dos dois existir para aquele marco, ninguem recebe o aviso automatico, mas o indicador visual no Painel de Prazos continua aparecendo normalmente."
Nota "O campo `Responsavel (conta)` e nao a pessoa preenche o texto livre `Responsavel` ja existente: e um segundo campo, um seletor com as contas que fazem parte da equipe do projeto selecionado. O texto livre `Responsavel` continua existindo do lado, sem nenhuma mudanca - o campo novo serve so para o sistema saber para qual conta mandar o aviso automatico."
